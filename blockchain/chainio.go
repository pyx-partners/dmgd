...
var (
	...
	// keySetBucketName is the name of the db bucket used to house the
	// admin key sets.
	keySetBucketName = []byte("keyset")
...
//  - compressed txout 0:
//    - 0x32: VLQ-encoded compressed amount for 5000000000 (5000 RMG)
...
//    - compressed txout:
//      - 0x91f20f: VLQ-encoded compressed amount for 34405000000 (34405 RMG)
...
//    - transaction version: 1
//    - compressed txout:
//      - 0x86c647: VLQ-encoded compressed amount for 13761000000 (13761 RMG)
//      - 0x00: special script type pay-to-pubkey-hash
//      - 0xb2...ec: pubkey hash
// -----------------------------------------------------------------------------
...
//  - compressed txout 0:
//    - 0x12: VLQ-encoded compressed amount for 20000000 (20 RMG)
//    - 0x00: special script type pay-to-pubkey-hash
//    - 0xe2...8a: pubkey hash
//  - compressed txout 2:
//    - 0x8009: VLQ-encoded compressed amount for 15000000 (15 RMG)
//    - 0x00: special script type pay-to-pubkey-hash
//    - 0xb8...58: pubkey hash
//
// Example 3:
...
//    NOTE: It's +2 since the first two outputs are encoded in the header code
//  - compressed txout 22:
//    - 0x8ba5b9e763: VLQ-encoded compressed amount for 366875659 (366.875659 RMG)
...
func deserializeUtxoEntry(serialized []byte) (*UtxoEntry, error) {
	...
	// all of the utxos.
	entry := newUtxoEntry(int32(version), isCoinBase, uint32(blockHeight))
...
// the provided values.
func dbPutBlockIndex(dbTx database.Tx, hash *chainhash.Hash, height uint32) error {
...
// values.
func dbRemoveBlockIndex(dbTx database.Tx, hash *chainhash.Hash, height uint32) error {
...
func dbFetchHeightByHash(dbTx database.Tx, hash *chainhash.Hash) (uint32, error) {
	meta := dbTx.Metadata()
	hashIndex := meta.Bucket(hashIndexBucketName)
...
func dbFetchHashByHeight(dbTx database.Tx, height uint32) (*chainhash.Hash, error) {
	var serializedHeight [4]byte
	byteOrder.PutUint32(serializedHeight[:], height)
...
// The sets are ROOT, PROVISION, ISSUE, ASP, and VALIDATE keys.
//
//
// The serialized format is:
//
...
//   ASP keys length       uint32      4 bytes
//   keyID / ASP keys      []pairs     Pair length * 37
...
func serializeKeySet(adminKeySets map[btcec.KeySetType]btcec.PublicKeySet,
	...
	// next. So we use keySetOrder to do a sorted itteration.
	for _, keySet := range adminKeysOrder {
		setLength := len(adminKeySets[keySet])
		byteOrder.PutUint32(serializedData[offset:], uint32(setLength))
		offset += 4
		for _, key := range adminKeySets[keySet] {
			copy(serializedData[offset:], key.SerializeCompressed())
			offset += btcec.PubKeyBytesLenCompressed
		}
	}
	// Serialize keyID to ASP Key map
	byteOrder.PutUint32(serializedData[offset:], uint32(len(aspKeyIdMap)))
	offset += 4

	// To have a deterministic order of serialization of a map, we take all keys
	// and sort them by keyID, then serialize in that order.
	var keyIDs []int
	for k := range aspKeyIdMap {
		keyIDs = append(keyIDs, int(k))
	}
	sort.Ints(keyIDs)
	for _, keyID := range keyIDs {
		pubKey := aspKeyIdMap[btcec.KeyID(keyID)]
...
// deserializeKeySet deserializes the passed serialized best chain
...
func deserializeKeySet(serializedData []byte) (
	...
	for _, keySet := range adminKeysOrder {
		...
		keySetLength := byteOrder.Uint32(serializedData[offset : offset+4])
		offset += 4
		...
		adminKeys[keySet] = make([]btcec.PublicKey, keySetLength)
		for i := 0; i < int(keySetLength); i++ {
			pubKey, _ := btcec.ParsePubKey(
				serializedData[offset:offset+btcec.PubKeyBytesLenCompressed], btcec.S256())
			adminKeys[keySet][i] = *pubKey
			offset += btcec.PubKeyBytesLenCompressed
		}
	}
	...
	// Deserialize keyIDs
	keyIdMapLen := byteOrder.Uint32(serializedData[offset : offset+4])
	offset += 4
	// Ensure the serialized data has enough bytes to deserialize the keys
...
func (b *BlockChain) initChainState() error {
	...
	err := b.db.View(func(dbTx database.Tx) error {
		...
		// Fetch the admin keys from the database.
		serializedKeys := dbTx.Metadata().Get(keySetBucketName)
		...
		header := &block.Header
		header.Height = uint32(state.height)
...
func dbFetchHeaderByHeight(dbTx database.Tx, height uint32) (*wire.BlockHeader, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockHashByHeight(blockHeight uint32) (*chainhash.Hash, error) {
	var hash *chainhash.Hash
	err := b.db.View(func(dbTx database.Tx) error {
		var err error
...
//
// This function is safe for concurrent access.
func (b *BlockChain) HeightRange(startHeight, endHeight uint32) ([]chainhash.Hash, error) {
