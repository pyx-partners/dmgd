...
type Config struct {
	...
	// utxo view.
	CalcSequenceLock func(*provautil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
func (mp *TxPool) removeOrphan(tx *provautil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *provautil.Tx) {
	mp.mtx.Lock()
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*provautil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *provautil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *provautil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *provautil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *provautil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*provautil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
func (mp *TxPool) processOrphans(acceptedTx *provautil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*provautil.Tx)
