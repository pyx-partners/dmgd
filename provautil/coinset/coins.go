...
func (cs *CoinSet) TotalValue() (value provautil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue provautil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue provautil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount provautil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue provautil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue provautil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue provautil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue provautil.Amount, coins []Coin) (Coins, error) {
...
// provautil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() provautil.Amount {
	return provautil.Amount(c.txOut().Value)
}
