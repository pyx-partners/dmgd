...
func TestBlock(t *testing.T) {
	b := provautil.NewBlock(&Block100000)
	...
	wantHashStr := "730785803d7a7dee39c7909b08cb35207de8cf70c762ffa62bfa97907b2c0573"
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := provautil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := provautil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := provautil.OutOfRangeError(wantErr)
	...
	_, err = provautil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	if _, ok := err.(provautil.OutOfRangeError); !ok {
		t.Errorf("TxHashWithSig: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, provautil.OutOfRangeError(""))
	}
