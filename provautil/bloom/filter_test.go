...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := provautil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(provautil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := provautil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := provautil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
